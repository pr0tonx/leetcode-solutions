...
on the preceding element. This result is achieved through the following operations: val = fn(init, nums[0]),
val = fn(val, nums[1]),val = fn(val, nums[2]), ... until every element in the array has been processed. The ultimate
...
const reduce = function (nums, fn, init) {
    ...
    for (let i = 0; i < nums.length; i++) accum = fn(accum, nums[i]);
