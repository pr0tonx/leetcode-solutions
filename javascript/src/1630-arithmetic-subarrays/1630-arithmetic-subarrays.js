...
const checkArithmeticSubarrays = function (nums, l, r) {
  ...
  for (let i = 0; i < l.length; i++) {
    ...
    const arrToCheck = nums.slice(lower, upper + 1).sort((a, b) => a - b);

    let isArithmetic = true;
    let diff = arrToCheck[1] - arrToCheck[0];
    for (let j = 0; j < arrToCheck.length - 1; j++) {
      const nextDiff = arrToCheck[j + 1] - arrToCheck[j];
